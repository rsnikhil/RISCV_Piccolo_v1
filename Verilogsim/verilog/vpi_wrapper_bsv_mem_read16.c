 ...
 *     #include "vpi_wrapper_bsv_mem_read16.h"
 *     void (*vlog_startup_routines[])() = { bsv_mem_read16_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bsv_mem_read16 call=bsv_mem_read16_calltf size=16 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bsv_mem_read16 vpiSysFuncSized 16 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned int bsv_mem_read16(unsigned long long , unsigned long long , unsigned long long );
...
PLI_INT32 bsv_mem_read16_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = bsv_mem_read16(arg_1, arg_2, arg_3);
...
/* sft: $imported_bsv_mem_read16 vpiSysFuncSized 16 unsigned */

/* tab: $imported_bsv_mem_read16 call=bsv_mem_read16_calltf size=16 acc=rw:%TASK */

PLI_INT32 bsv_mem_read16_sizetf(PLI_BYTE8 *user_data)
{
  return 16u;
}
...
void bsv_mem_read16_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bsv_mem_read16";
  tf_data.calltf = bsv_mem_read16_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bsv_mem_read16_sizetf;
  tf_data.user_data = "$imported_bsv_mem_read16";
