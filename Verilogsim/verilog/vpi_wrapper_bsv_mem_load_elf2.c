 ...
 *     void (*vlog_startup_routines[])() = { bsv_mem_load_elf2_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bsv_mem_load_elf2 call=bsv_mem_load_elf2_calltf size=32 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bsv_mem_load_elf2 vpiSysFuncSized 32 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned int bsv_mem_load_elf2(unsigned long long , unsigned long long );
...
PLI_INT32 bsv_mem_load_elf2_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = bsv_mem_load_elf2(arg_1, arg_2);
...
/* sft: $imported_bsv_mem_load_elf2 vpiSysFuncSized 32 unsigned */

/* tab: $imported_bsv_mem_load_elf2 call=bsv_mem_load_elf2_calltf size=32 acc=rw:%TASK */

PLI_INT32 bsv_mem_load_elf2_sizetf(PLI_BYTE8 *user_data)
{
  return 32u;
}
...
void bsv_mem_load_elf2_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bsv_mem_load_elf2";
  tf_data.calltf = bsv_mem_load_elf2_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bsv_mem_load_elf2_sizetf;
  tf_data.user_data = "$imported_bsv_mem_load_elf2";
