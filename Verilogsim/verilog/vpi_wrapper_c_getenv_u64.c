 ...
 *     void (*vlog_startup_routines[])() = { c_getenv_u64_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_c_getenv_u64 call=c_getenv_u64_calltf size=64 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_c_getenv_u64 vpiSysFuncSized 64 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned long long c_getenv_u64(char *);

/* VPI wrapper function */
PLI_INT32 c_getenv_u64_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  char *arg_1;
...
/* sft: $imported_c_getenv_u64 vpiSysFuncSized 64 unsigned */

/* tab: $imported_c_getenv_u64 call=c_getenv_u64_calltf size=64 acc=rw:%TASK */

PLI_INT32 c_getenv_u64_sizetf(PLI_BYTE8 *user_data)
{
  return 64u;
}

/* VPI wrapper registration function */
void c_getenv_u64_vpi_register()
{
  ...
  tf_data.tfname = "$imported_c_getenv_u64";
  tf_data.calltf = c_getenv_u64_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = c_getenv_u64_sizetf;
  tf_data.user_data = "$imported_c_getenv_u64";
