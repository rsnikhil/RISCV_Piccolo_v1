 ...
 *     void (*vlog_startup_routines[])() = { bsv_mem_get_exit_pc_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bsv_mem_get_exit_pc call=bsv_mem_get_exit_pc_calltf size=64 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bsv_mem_get_exit_pc vpiSysFuncSized 64 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned long long bsv_mem_get_exit_pc(unsigned long long );

/* VPI wrapper function */
PLI_INT32 bsv_mem_get_exit_pc_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  ...
  vpi_result = bsv_mem_get_exit_pc(arg_1);
...
/* sft: $imported_bsv_mem_get_exit_pc vpiSysFuncSized 64 unsigned */

/* tab: $imported_bsv_mem_get_exit_pc call=bsv_mem_get_exit_pc_calltf size=64 acc=rw:%TASK */

PLI_INT32 bsv_mem_get_exit_pc_sizetf(PLI_BYTE8 *user_data)
{
  return 64u;
}

/* VPI wrapper registration function */
void bsv_mem_get_exit_pc_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bsv_mem_get_exit_pc";
  tf_data.calltf = bsv_mem_get_exit_pc_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bsv_mem_get_exit_pc_sizetf;
  tf_data.user_data = "$imported_bsv_mem_get_exit_pc";
